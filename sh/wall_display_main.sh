...
blue="#132492"
white="#ffffff"
( sh/bottom.caption.sh | sh/bar.sh -f "helv:size=34:weight=bold:antialias=true" -n "bottom_photo_caption" -B $blue -F $white -g "1920x120+0+$((1080 - 120 - 20))"  || : ) &
...
while sh/is.running ; do
  ...
  for x in $(find wall_display/special -type f -iname "store.jpg"  -or -iname "stroganoff.jpg" ) ; do
    ...
    if ! test -z "$caption" ; then
      ( echo "%{c}%{T2}$caption" | sh/bar.sh -n "top_photo_caption" -B $blue -F $white -o -7 || : ) &
    fi
    sh/sleep.minute
    pkill -INT -f top_photo_caption || :
  done
